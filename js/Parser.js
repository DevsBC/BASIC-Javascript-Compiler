...
Tree.prototype.eat = function(c) {
	for(var line in c) {
		...
		switch(command.toUpperCase()) {
			...
			case "COMMENT":
				break;
			default:
				addError( line_num, posi_num, "se esperaba un comando pero se encontro: " + command);
		}
	}
	...
	if(this.rootType.length>0) {
		...
		if(remain.value==="else") {
			...
			addError( remain.line, remain.posi, "El comando " + remainCommand + " le falta END" + remainCommand);
		}
		else {
			addError( remain.line, remain.posi, "El comando " + remainCommand + " le falta END" + remainCommand);
		}
...
Tree.prototype.eatNumber = function(lineArr) {	
	if(lineArr.length<2) {
		addError(lineArr[0][1], lineArr[0][2], "el comando NUMBER no esta completo");
		return;
	}
	else {
	...
	if(lineArr.length>2) {
		if(lineArr[2][0]!=='=') {
			addError(lineArr[2][1], lineArr[2][2], "Comando invalido para NUMBER, variable name should be followed by =");
		}
